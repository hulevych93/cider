...
static void build_document_order(std::vector<pugi::xpath_node>& result,
                                 pugi::xml_node root) {
...
bool test_xpath_string(const pugi::xpath_node& node,
  ...
  return q.evaluate_string(&buffer[0], size, node) == size &&
         test_string_equal(buffer.c_str(), expected);
}
...
bool test_xpath_number(const pugi::xpath_node& node,
  ...
  return absolute_error < tolerance ||
         absolute_error < fabs(expected) * tolerance;
}
...
xpath_node_set_tester::xpath_node_set_tester(const pugi::xpath_node_set& set,
                                             const char* message_)
    : last(0), message(message_) {
  result = set;

  // only sort unsorted sets so that we're able to verify reverse order for some
  // axes
  if (result.type() == pugi::type_unsorted)
    result.sort();
  ...
  if (result.empty()) {
    ...
    build_document_order(
        order,
        (result[0].attribute() ? result[0].parent() : result[0].node()).root());
