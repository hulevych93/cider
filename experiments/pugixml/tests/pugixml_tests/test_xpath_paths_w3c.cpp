...
TEST_XML(
  ...
  CHECK_XPATH_NODESET(n.child(STR("para")), STR("descendant::para")) % 4 % 5 %
      7;
}

TEST_XML(xpath_paths_w3c_8,
         "<node><div><font><div><div/></div></font></div></node>") {
  xml_node c;
  xml_node n = doc.child(STR("node"));

  CHECK_XPATH_NODESET(c, STR("ancestor::div"));
  CHECK_XPATH_NODESET(n.child(STR("div"))
                          .child(STR("font"))
                          .child(STR("div"))
                          .child(STR("div")),
                      STR("ancestor::div")) %
...
TEST_XML(
  ...
  CHECK_XPATH_NODESET(n.child(STR("para")), STR("descendant-or-self::para")) %
      3 % 4 % 5 % 7;
}
...
TEST_XML(xpath_paths_w3c_12,
         "<chapter><para><para/><para/><foo><para/></foo></para><foo/><para/></"
         "chapter>") {
  xml_node c;

  CHECK_XPATH_NODESET(c, STR("child::chapter/descendant::para"));
  CHECK_XPATH_NODESET(doc, STR("child::chapter/descendant::para")) % 3 % 4 % 5 %
      7 % 9;
}
...
TEST_XML(
  ...
  CHECK_XPATH_NODESET(c, STR("/descendant::para"));
  CHECK_XPATH_NODESET(n, STR("/descendant::para")) % 3 % 4 % 5 % 7 % 9;
  CHECK_XPATH_NODESET(n.child(STR("para")), STR("/descendant::para")) % 3 % 4 %
      5 % 7 % 9;
}

TEST_XML(xpath_paths_w3c_16,
  ...
  CHECK_XPATH_NODESET(n.child(STR("olist")),
                      STR("/descendant::olist/child::item")) %
      4 % 8 % 9;
}
...
TEST_XML(xpath_paths_w3c_23,
         "<node><figure><figure/><figure/><foo><figure/></foo></figure><foo/"
         "><figure/></node>") {
  xml_node c;
  xml_node n = doc.child(STR("node"));
  ...
  CHECK_XPATH_NODESET(n.child(STR("figure")),
                      STR("/descendant::figure[position()=4]")) %
      7;
}

TEST_XML(xpath_paths_w3c_24,
  ...
  CHECK_XPATH_NODESET(c, STR("/child::doc/child::chapter[position()=5]/"
                             "child::section[position()=2]"));
  CHECK_XPATH_NODESET(n, STR("/child::doc/child::chapter[position()=5]/"
                             "child::section[position()=2]")) %
      9;
  CHECK_XPATH_NODESET(doc, STR("/child::doc/child::chapter[position()=5]/"
                               "child::section[position()=2]")) %
      9;
}
...
TEST_XML(
  ...
  CHECK_XPATH_NODESET(n, STR("child::para[attribute::type=\"warning\"]")) % 4 %
      6 % 11 % 13 % 15;
}
...
TEST_XML(
  ...
  CHECK_XPATH_NODESET(
      c, STR("child::para[attribute::type=\"warning\"][position()=5]"));
  CHECK_XPATH_NODESET(
      n, STR("child::para[attribute::type=\"warning\"][position()=5]")) %
      15;
}
...
TEST_XML(
  ...
  CHECK_XPATH_NODESET(
      c, STR("child::para[position()=5][attribute::type=\"warning\"]"));
  CHECK_XPATH_NODESET(
      n, STR("child::para[position()=5][attribute::type=\"warning\"]"));
}
...
TEST_XML(
  ...
  CHECK_XPATH_NODESET(n, STR("child::chapter[child::title='Introduction']")) %
      6 % 13;
}
...
TEST_XML(
    xpath_paths_w3c_30,
    "<node><abstract/><chapter/><chapter/><references/><appendix/></node>") {
  xml_node c;
  xml_node n = doc.child(STR("node"));

  CHECK_XPATH_NODESET(c, STR("child::*[self::chapter or self::appendix]"));
  CHECK_XPATH_NODESET(n, STR("child::*[self::chapter or self::appendix]")) % 4 %
      5 % 7;
}
...
TEST_XML(
  ...
  CHECK_XPATH_NODESET(
      c, STR("child::*[self::chapter or self::appendix][position()=last()]"));
  CHECK_XPATH_NODESET(
      n, STR("child::*[self::chapter or self::appendix][position()=last()]")) %
      7;
}

TEST_XML(xpath_paths_w3c_31b,
         "<node><abstract/><chapter/><chapter/><references/><appendix/"
         "><chapter/></node>") {
  xml_node c;
  xml_node n = doc.child(STR("node"));
