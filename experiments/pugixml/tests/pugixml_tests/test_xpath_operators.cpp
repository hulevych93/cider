...
TEST_XML(
  ...
  CHECK_XPATH_BOOLEAN(
      n, STR("c1/v != c5/v"),
      true);  // (a, b) != (a, b), since a != b, as per XPath spec (comparison
              // operators are so not intuitive)
...
TEST(xpath_operators_mod) {
  // Check that mod operator conforms to Java spec (since this is the only
  // concrete source of information about XPath mod)
  xml_node c;
  ...
  // In the remaining cases ...  the floating-point remainder r from the
  // division of a dividend n by a divisor d is defined by the mathematical
  // relation r = n - (d * q) where q is an integer that is negative only if n/d
  // is negative and positive only if n/d is positive, and whose magnitude is as
  ...
  CHECK_XPATH_NUMBER(c,
                     STR("11579208923731619542357098500868790785326998466564056"
                         "4039457584007913129639935 mod 2"),
                     0);
  CHECK_XPATH_NUMBER(c,
                     STR("11579208923731619542357098500868790785326998466564056"
                         "4039457584007913129639935 mod 3"),
                     1);
}
#endif
