...
inline bool test_node_name_value(const Node& node,
                                 const pugi::char_t* name,
                                 const pugi::char_t* value) {
  return test_string_equal(node.name(), name) &&
...
bool test_xpath_string(const pugi::xpath_node& node,
                       const pugi::char_t* query,
                       pugi::xpath_variable_set* variables,
                       const pugi::char_t* expected);
bool test_xpath_boolean(const pugi::xpath_node& node,
                        const pugi::char_t* query,
                        pugi::xpath_variable_set* variables,
                        bool expected);
bool test_xpath_number(const pugi::xpath_node& node,
                       const pugi::char_t* query,
                       pugi::xpath_variable_set* variables,
                       double expected);
bool test_xpath_number_nan(const pugi::xpath_node& node,
                           const pugi::char_t* query,
                           pugi::xpath_variable_set* variables);
...
#define CHECK_TEXT(condition, text)                                        \
  if (condition)                                                           \
    ...
    test_runner::_failure_message = CHECK_JOIN2(text, __FILE__, __LINE__), \
    longjmp(test_runner::_failure_buffer, 1)
#define CHECK_FORCE_FAIL(text)                                           \
  test_runner::_failure_message = CHECK_JOIN2(text, __FILE__, __LINE__), \
  longjmp(test_runner::_failure_buffer, 1)

#if (defined(_MSC_VER) && _MSC_VER == 1200) || defined(__MWERKS__) || \
    (defined(__BORLANDC__) && __BORLANDC__ <= 0x540)
#define STRINGIZE(value) \
  "??"  // Some compilers have issues with stringizing expressions that contain
        // strings w/escaping inside
#else
...
#define CHECK_STRING(value, expected)            \
  CHECK_TEXT(test_string_equal(value, expected), \
             STRINGIZE(value) " is not equal to " STRINGIZE(expected))
#define CHECK_DOUBLE(value, expected)                                         \
  CHECK_TEXT((value > expected ? value - expected : expected - value) < 1e-6, \
             STRINGIZE(value) " is not equal to " STRINGIZE(expected))
#define CHECK_DOUBLE_NAN(value) \
  CHECK_TEXT(test_double_nan(value), STRINGIZE(value) " is not equal to NaN")
#define CHECK_NAME_VALUE(node, name, value)           \
  CHECK_TEXT(test_node_name_value(node, name, value), \
             STRINGIZE(node) " name/value do not match " STRINGIZE(name) " and " STRINGIZE(value))
...
#define CHECK_NODE(node, expected) \
  CHECK_NODE_EX(node, expected, PUGIXML_TEXT(""), pugi::format_raw)
...
#define CHECK_XPATH_NUMBER_VAR(node, query, variables, expected)  \
  CHECK_TEXT(test_xpath_number(node, query, variables, expected), \
             STRINGIZE(query) " does not evaluate to " STRINGIZE(expected) " in context " STRINGIZE(node))
#define CHECK_XPATH_NUMBER_NAN_VAR(node, query, variables) \
  CHECK_TEXT(                                              \
      test_xpath_number_nan(node, query, variables),       \
...
#define CHECK_XPATH_FAIL_VAR(query, variables)          \
  CHECK_TEXT(test_xpath_fail_compile(query, variables), \
             STRINGIZE(query) " should not compile")

#define CHECK_XPATH_STRING(node, query, expected) \
  CHECK_XPATH_STRING_VAR(node, query, 0, expected)
#define CHECK_XPATH_BOOLEAN(node, query, expected) \
  CHECK_XPATH_BOOLEAN_VAR(node, query, 0, expected)
#define CHECK_XPATH_NUMBER(node, query, expected) \
  CHECK_XPATH_NUMBER_VAR(node, query, 0, expected)
#define CHECK_XPATH_NUMBER_NAN(node, query) \
  CHECK_XPATH_NUMBER_NAN_VAR(node, query, 0)
...
#define U_LITERALS  // DMC does not understand \x01234 (it parses first three
                    // digits), but understands \u01234
#endif

#if (defined(_MSC_VER) && _MSC_VER == 1200) ||                \
    (defined(__INTEL_COMPILER) && __INTEL_COMPILER == 800) || \
    defined(__BORLANDC__)
// NaN comparison on MSVC6 is incorrect, see
// http://www.nabble.com/assertDoubleEquals,-NaN---Microsoft-Visual-Studio-6-td9137859.html
