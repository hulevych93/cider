...
TEST(xpath_parse_paths_valid) {
  const char_t* paths[] = {
      ...
      // Oasis MSFT considers this path invalid, however I believe it's valid as
      // per spec
      STR("**..**"),
...
TEST(xpath_parse_paths_valid_unicode) {
  ...
  const wchar_t* paths[] = {
      ...
      L"//*[@\u30c7\u30b9\u30c6\u30a3\u30cd\u30a4\u30b7\u30e7\u30f3]",
      L"\u30da\u30fc\u30b8 = '\u304b\u3089'",
      L"concat(substring-before(@\u30a4\u30e1\u30fc\u30b8,'\u76ee\u5370'),'"
      L"\u30a2\u30a4\u30b3\u30f3',substring-after(@\u30a4\u30e1\u30fc\u30b8,'"
      L"\u76ee\u5370'))",
      L"\u30bd\u30fc\u30b9|"
      L"\u30c7\u30b9\u30c6\u30a3\u30cd\u30a4\u30b7\u30e7\u30f3",
      L"\u30da\u30fc\u30b8 != '\u307e\u3067' and \u30da\u30fc\u30b8 != "
      L"'\u304b\u3089'",
      L"substring-after(\u30a2\u30a4\u30b3\u30f3/@\u30a4\u30e1\u30fc\u30b8, "
      L"'/\u5730\u56f3\u30d5\u30a1\u30a4\u30eb/\u76ee\u5370')",
      L"child::\u6bb5\u843d",
      ...
      L"/child::\u6587\u66f8/child::\u7ae0[position()=5]/"
      L"child::\u7bc0[position()=2]",
      L"child::\u6bb5\u843d[attribute::\u30bf\u30a4\u30d7='\u8b66\u544a']",
      L"child::\u6bb5\u843d[attribute::\u30bf\u30a4\u30d7='\u8b66\u544a']["
      L"position()=5]",
      L"child::\u6bb5\u843d[position()=5][attribute::\u30bf\u30a4\u30d7='"
      L"\u8b66\u544a']",
      ...
      L"concat(substring-before(@\x30a4\x30e1\x30fc\x30b8,'\x76ee\x5370'),'"
      L"\x30a2\x30a4\x30b3\x30f3',substring-after(@\x30a4\x30e1\x30fc\x30b8,'"
      L"\x76ee\x5370'))",
      L"\x30bd\x30fc\x30b9|"
      L"\x30c7\x30b9\x30c6\x30a3\x30cd\x30a4\x30b7\x30e7\x30f3",
      L"\x30da\x30fc\x30b8 != '\x307e\x3067' and \x30da\x30fc\x30b8 != "
      L"'\x304b\x3089'",
      L"substring-after(\x30a2\x30a4\x30b3\x30f3/@\x30a4\x30e1\x30fc\x30b8, "
      L"'/\x5730\x56f3\x30d5\x30a1\x30a4\x30eb/\x76ee\x5370')",
      L"child::\x6bb5\x843d",
      ...
      L"/child::\x6587\x66f8/child::\x7ae0[position()=5]/"
      L"child::\x7bc0[position()=2]",
      L"child::\x6bb5\x843d[attribute::\x30bf\x30a4\x30d7='\x8b66\x544a']",
      L"child::\x6bb5\x843d[attribute::\x30bf\x30a4\x30d7='\x8b66\x544a']["
      L"position()=5]",
      L"child::\x6bb5\x843d[position()=5][attribute::\x30bf\x30a4\x30d7='"
      L"\x8b66\x544a']",
      L"child::\x7ae0[child::\x30bf\x30a4\x30c8\x30eb='"
      L"\x306f\x3058\x3081\x306b']",
      L"child::\x7ae0[child::\x30bf\x30a4\x30c8\x30eb]",
...
TEST(xpath_parse_out_of_memory_second_page_node) {
  ...
  CHECK_ALLOC_FAIL(CHECK_XPATH_FAIL(
...
TEST(xpath_parse_error_propagation) {
  char_t query[] =
      STR("(//foo[count(. | @*)] | ((a)//b)[1] | /foo | "
          "/foo/bar//more/ancestor-or-self::foobar | /text() | a[1 + 2 * 3 div "
          "(1+0) mod 2]//b[1]/c | a[$x])[true()]");
...
TEST(xpath_parse_oom_propagation) {
  ...
  for (size_t i = 3200; i < 4200; ++i) {
    ...
    std::basic_string<char_t> query =
        STR("processing-instruction('") + literal + STR("') | ") + query_base;

    CHECK_ALLOC_FAIL(CHECK_XPATH_FAIL(query.c_str()));
  }
}

static std::basic_string<char_t> rep(const std::basic_string<char_t>& base,
                                     size_t count) {
...
TEST(xpath_parse_depth_limit) {
  ...
  CHECK_XPATH_FAIL(
      (rep(STR("("), limit) + STR("1") + rep(STR(")"), limit)).c_str());
  ...
  CHECK_XPATH_FAIL(
      (STR("concat(") + rep(STR("1,"), limit) + STR("1)")).c_str());
