...
TEST(xpath_number_ceiling) {
  ...
  // ceiling with argument in range (-1, -0] should result in minus zero
    ...
    !defined(__CLR_VER)  // MacOS X gcc 4.0.1 and x64 CLR implement ceil
                         // incorrectly (ceil never returns -0)
  CHECK_XPATH_STRING(c, STR("string(1 div ceiling(-0))"), STR("-Infinity"));
...
TEST_XML(xpath_boolean_lang,
         "<node xml:lang='en'><child xml:lang='zh-UK'><subchild "
         "attr=''/></child></node><foo><bar/></foo>") {
  xml_node c;
...
TEST(xpath_string_translate_table) {
  ...
  CHECK_XPATH_STRING(c, STR("translate('abcd\xe9 ', 'abc', 'ABC')"),
                     STR("ABCd\xe9 "));
  CHECK_XPATH_STRING(c, STR("translate('abcd\xe9 ', 'abc\xe9', 'ABC!')"),
                     STR("ABCd! "));
  CHECK_XPATH_STRING(c, STR("translate('abcd! ', 'abc!', 'ABC\xe9')"),
                     STR("ABCd\xe9 "));
  CHECK_XPATH_STRING(c, STR("translate('abcde', concat('abc', 'd'), 'ABCD')"),
                     STR("ABCDe"));
  CHECK_XPATH_STRING(c, STR("translate('abcde', 'abcd', concat('ABC', 'D'))"),
                     STR("ABCDe"));
}
...
TEST(xpath_string_translate_remove) {
  ...
  CHECK_XPATH_STRING(c, STR("translate('000000755', concat('0', ''), '')"),
                     STR("755"));
}
