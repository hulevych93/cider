...
TEST_XML(write_indent_attributes,
         "<node attr='1' other='2'><child><sub>text</sub></child></node>") {
  CHECK_NODE_EX(doc,
                STR("<node\n\tattr=\"1\"\n\tother=\"2\">\n\t<child>\n\t\t<sub>"
                    "text</sub>\n\t</child>\n</node>\n"),
...
TEST_XML_FLAGS(
    write_indent_attributes_declaration,
    "<?xml version=\"1.0\" encoding=\"UTF-8\"?><node attr='1' other='2' />",
    parse_full) {
  CHECK_NODE_EX(
      doc,
      STR("<?xml version=\"1.0\" "
...
TEST_XML(write_indent_attributes_raw,
         "<node attr='1' other='2'><child><sub>text</sub></child></node>") {
  CHECK_NODE_EX(
      doc,
      STR("<node attr=\"1\" other=\"2\"><child><sub>text</sub></child></node>"),
      STR("\t"), format_indent_attributes | format_raw);
}

TEST_XML(write_indent_attributes_empty_indent,
         "<node attr='1' other='2'><child><sub>text</sub></child></node>") {
  CHECK_NODE_EX(doc,
                STR("<node\nattr=\"1\"\nother=\"2\">\n<child>\n<sub>text</"
                    "sub>\n</child>\n</node>\n"),
...
TEST_XML_FLAGS(write_cdata_empty,
               "<![CDATA[]]>",
...
TEST_XML_FLAGS(write_cdata_escape,
               "<![CDATA[value]]>",
               parse_cdata | parse_fragment) {
  CHECK_NODE(doc, STR("<![CDATA[value]]>"));
...
TEST_XML_FLAGS(write_declaration,
               "<?xml version='2.0'?>",
...
TEST_XML(write_escape_roundtrip, "<node attr=''>text</node>") {
  ...
  // Note: this string is almost identical to the string from write_escape with
  // the exception of \r \r in PCDATA doesn't roundtrip because it has to go
  // through newline conversion (which could be disabled, but is active by
  // default)
  CHECK_NODE(doc, STR("<node "
                      "attr=\"&lt;>'&quot;&amp;&#04;&#13;&#10;&#09;\">&lt;&gt;'"
                      "\"&amp;&#04;\n\t</node>"));
...
TEST(write_encoding_huge) {
  ...
  // make a large utf16 name consisting of 6-byte char pairs (6 does not divide
...
TEST(write_encoding_huge) {
  ...
  // make a large utf8 name consisting of 3-byte chars (3 does not divide
  // internal buffer size, so will need split correction)
  std::string s_utf8 = "<";
...
TEST(write_no_name_element) {
  ...
  CHECK_NODE(doc, STR("<:anonymous><:anonymous/><:anonymous>text</:anonymous></"
                      ":anonymous>"));
  CHECK_NODE_EX(doc,
                STR("<:anonymous>\n\t<:anonymous "
...
TEST_XML(write_indent_custom,
         "<node attr='1'><child><sub>text</sub></child></node>") {
  CHECK_NODE_EX(
      doc,
      STR("<node attr=\"1\">\n<child>\n<sub>text</sub>\n</child>\n</node>\n"),
      STR(""), format_indent);
...
TEST(write_pcdata_whitespace_fixedpoint) {
  ...
  static const unsigned int flags_parse[] = {
      0, parse_ws_pcdata, parse_ws_pcdata_single, parse_trim_pcdata};

  static const unsigned int flags_format[] = {0, format_raw, format_indent};

  for (unsigned int i = 0; i < sizeof(flags_parse) / sizeof(flags_parse[0]);
       ++i) {
    xml_document doc;
    CHECK(doc.load_string(data, flags_parse[i]));

    for (unsigned int j = 0; j < sizeof(flags_format) / sizeof(flags_format[0]);
         ++j) {
...
TEST_XML_FLAGS(
    write_mixed,
    "<node><child1/><child2>pre<![CDATA[data]]>mid<!--comment--><test/"
    ">post<?pi value?>fin</child2><child3/></node>",
    parse_full) {
  CHECK_NODE(
      doc,
      STR("<node><child1/><child2>pre<![CDATA[data]]>mid<!--comment--><test/"
          ">post<?pi value?>fin</child2><child3/></node>"));
  CHECK_NODE_EX(doc,
                STR("<node>\n<child1 "
                    "/>\n<child2>pre<![CDATA[data]]>mid<!--comment-->\n<test "
                    "/>post<?pi value?>fin</child2>\n<child3 />\n</node>\n"),
                STR("\t"), 0);
  CHECK_NODE_EX(
      doc,
      STR("<node>\n\t<child1 "
...
TEST_XML(write_no_empty_element_tags,
         "<node><child1/><child2>text</child2><child3></child3></node>") {
  CHECK_NODE(doc, STR("<node><child1/><child2>text</child2><child3/></node>"));
  CHECK_NODE_EX(doc,
                STR("<node><child1></child1><child2>text</child2><child3></"
                    "child3></node>"),
                STR("\t"), format_raw | format_no_empty_element_tags);
  CHECK_NODE_EX(doc,
                STR("<node>\n\t<child1></child1>\n\t<child2>text</"
                    "child2>\n\t<child3></child3>\n</node>\n"),
...
TEST_XML_FLAGS(write_roundtrip,
               "<node><child1 attr1='value1' attr2='value2'/><child2 "
               "attr='value'>pre<![CDATA[data]]>mid&lt;text&amp;escape<!--"
               "comment--><test/>post<?pi value?>fin</child2><child3/></node>",
               parse_full) {
  const unsigned int flagset[] = {format_indent,
                                  format_raw,
  ...
  for (size_t i = 0; i < (size_t(1) << flagcount); ++i) {
    ...
    CHECK(test_write_narrow(verify, flags, encoding_utf8, contents.c_str(),
                            contents.size()));

    xml_document verifyws;
    CHECK(verifyws.load_buffer(contents.c_str(), contents.size(),
                               parse_full | parse_ws_pcdata));
    CHECK(test_write_narrow(verifyws, flags, encoding_utf8, contents.c_str(),
                            contents.size()));
  }
}
...
TEST_XML(write_skip_control_chars, "<a>\f\t\n\x0F\x19</a>") {
  CHECK_NODE_EX(doc.first_child(), STR("<a>\t\n</a>\n"), STR(""),
                pugi::PugixmlHooked::format_default |
                    pugi::PugixmlHooked::format_skip_control_chars);
}
