...
TEST(parse_pi_skip) {
  ...
  unsigned int flag_sets[] = {parse_fragment,
...
TEST(parse_pi_error) {
  ...
  for (unsigned int i = 0; i < sizeof(flag_sets) / sizeof(flag_sets[0]); ++i) {
    ...
    CHECK(doc.load_string(STR("<?name value "), flags).status == status_bad_pi);
    CHECK(doc.load_string(STR("<?name value  "), flags).status ==
          status_bad_pi);
    CHECK(doc.load_string(STR("<?name value  ?"), flags).status ==
          status_bad_pi);
    CHECK(doc.load_string(STR("<?name value  ? "), flags).status ==
  ...
  CHECK(doc.load_string(STR("<?xx#?>"), parse_fragment | parse_pi).status ==
        status_bad_pi);
  CHECK(doc.load_string(STR("<?name&?>"), parse_fragment | parse_pi).status ==
        status_bad_pi);
  CHECK(doc.load_string(STR("<?name& x?>"), parse_fragment | parse_pi).status ==
        status_bad_pi);
}
...
TEST(parse_comments_error) {
  ...
  unsigned int flag_sets[] = {parse_fragment, parse_fragment | parse_comments,
                              parse_fragment | parse_comments | parse_eol};
  ...
  for (unsigned int i = 0; i < sizeof(flag_sets) / sizeof(flag_sets[0]); ++i) {
    ...
    CHECK(doc.load_string(STR("<!-->"), flags).status == status_bad_comment);
    CHECK(doc.load_string(STR("<!--->"), flags).status == status_bad_comment);
    CHECK(doc.load_string(STR("<!-- <!-- --><!- -->"), flags).status ==
          status_bad_comment);
  }
}
...
TEST(parse_cdata_error) {
  ...
  unsigned int flag_sets[] = {parse_fragment, parse_fragment | parse_cdata,
                              parse_fragment | parse_cdata | parse_eol};
  ...
  for (unsigned int i = 0; i < sizeof(flag_sets) / sizeof(flag_sets[0]); ++i) {
    ...
    CHECK(doc.load_string(STR("<![CDATA["), flags).status == status_bad_cdata);
    CHECK(doc.load_string(STR("<![CDATA[]"), flags).status == status_bad_cdata);
    CHECK(doc.load_string(STR("<![CDATA[data"), flags).status ==
          status_bad_cdata);
    CHECK(doc.load_string(STR("<![CDATA[data]"), flags).status ==
          status_bad_cdata);
    CHECK(doc.load_string(STR("<![CDATA[data]]"), flags).status ==
          status_bad_cdata);
    CHECK(doc.load_string(STR("<![CDATA[>"), flags).status == status_bad_cdata);
    CHECK(doc.load_string(STR("<![CDATA[ <![CDATA[]]><![CDATA ]]>"), flags)
              .status == status_bad_cdata);
  }
}
...
TEST(parse_ws_pcdata_permutations) {
  ...
  test_data_t test_data[] = {
      ...
      {7, STR("<node>inner1<child/>inner2</node>"),
       STR("<node>inner1<child/>inner2</node>"), 5},
      {7, STR("<node>inner1<child>deep</child>inner2</node>"),
       STR("<node>inner1<child>deep</child>inner2</node>"), 6},
      // empty pcdata nodes should never be created
      {7, STR("<node>inner1<child></child>inner2</node>"),
       STR("<node>inner1<child/>inner2</node>"), 5},
      {7, STR("<node><child></child>inner2</node>"),
       STR("<node><child/>inner2</node>"), 4},
      {7, STR("<node>inner1<child></child></node>"),
       STR("<node>inner1<child/></node>"), 4},
      {7, STR("<node><child></child></node>"), STR("<node><child/></node>"), 3},
      // comments, pi or other nodes should not cause pcdata creation either
      {7, STR("<node><!----><child><?pi?></child><![CDATA[x]]></node>"),
       STR("<node><child/><![CDATA[x]]></node>"), 4},
      // leading/trailing pcdata whitespace should be preserved (note: this will
      // change if parse_ws_pcdata_trim is introduced)
      {7, STR("<node>\t \tinner1<child> deep   </child>\t\ninner2\n\t</node>"),
       STR("<node>\t \tinner1<child> deep   </child>\t\ninner2\n\t</node>"), 6},
      // whitespace-only pcdata preservation depends on the parsing mode
      {1,
       STR("<node>\n\t<child>   </child>\n\t<child> <deep>  </deep> "
           "</child>\n\t<!---->\n\t</node>"),
       STR("<node><child/><child><deep/></child></node>"), 5},
      {2,
       STR("<node>\n\t<child>   </child>\n\t<child> <deep>  </deep> "
           "</child>\n\t<!---->\n\t</node>"),
       STR("<node>\n\t<child>   </child>\n\t<child> <deep>  </deep> "
      ...
      {4,
       STR("<node>\n\t<child>   </child>\n\t<child> <deep>  </deep> "
           "</child>\n\t<!---->\n\t</node>"),
       STR("<node><child>   </child><child><deep>  </deep></child></node>"), 7},
      // current implementation of parse_ws_pcdata_single has an unfortunate
      // bug; reproduce it here
      {4, STR("<node>\t\t<!---->\n\n</node>"), STR("<node>\n\n</node>"), 3},
  ...
  for (size_t i = 0; i < sizeof(test_data) / sizeof(test_data[0]); ++i) {
    ...
    for (int flag = 0; flag < 3; ++flag) {
      if (td.mask & (1 << flag)) {
        unsigned int flags[] = {parse_default, parse_default | parse_ws_pcdata,
...
TEST(parse_ws_pcdata_fragment_permutations) {
  ...
  test_data_t test_data[] = {
      ...
      {7, STR("ext1<node1/>ext2<node2/>ext3"),
       STR("ext1<node1/>ext2<node2/>ext3"), 6},
      {5, STR(" <node/>"), STR("<node/>"), 2},
...
TEST(parse_pcdata_error) {
  xml_document doc;
  CHECK(doc.load_string(STR("<root>pcdata"), parse_minimal).status ==
...
TEST(parse_pcdata_trim) {
  ...
  test_data_t test_data[] = {
      ...
      {STR("<node>     test&amp;&amp;&amp;&amp;&amp;&amp;&amp;    </node>"),
       STR("test&amp;&amp;&amp;&amp;&amp;&amp;&amp;"), 0},
      {STR("<node>     test&amp;&amp;&amp;&amp;&amp;&amp;&amp;    </node>"),
       STR("test&&&&&&&"), parse_escapes},
      {STR("     test&amp;&amp;&amp;&amp;&amp;&amp;&amp;    "),
       STR("test&&&&&&&"), parse_fragment | parse_escapes},
      {STR("<node>\r\n\t text \t\n\r m&amp;&amp;e \r\n\t</node>"),
...
TEST(parse_pcdata_trim_empty) {
  unsigned int flags[] = {0, parse_ws_pcdata, parse_ws_pcdata_single,
                          parse_ws_pcdata | parse_ws_pcdata_single};
...
TEST(parse_escapes_unicode) {
  ...
  CHECK(v[0] == 0x3b3 && v[1] == 0x3b3 &&
        (wcharsize == 2
             ? v[2] == wchar_cast(0xd852) && v[3] == wchar_cast(0xdf62)
             : v[2] == wchar_cast(0x24b62)));
#else
  CHECK_STRING(doc.child_value(STR("node")),
               "\xce\xb3\xce\xb3\xf0\xa4\xad\xa2");
#endif
}
...
TEST(parse_attribute_error) {
  xml_document doc;
  CHECK(doc.load_string(STR("<node id"), parse_minimal).status ==
  ...
  CHECK(doc.load_string(STR("<node id='\"/>"), parse_minimal).status ==
        status_bad_attribute);
  CHECK(doc.load_string(STR("<node #/>"), parse_minimal).status ==
        status_bad_start_element);
  CHECK(doc.load_string(STR("<node#/>"), parse_minimal).status ==
        status_bad_start_element);
  CHECK(doc.load_string(STR("<node id1='1'id2='2'/>"), parse_minimal).status ==
        status_bad_attribute);
  CHECK(doc.load_string(STR("<node id&='1'/>"), parse_minimal).status ==
        status_bad_attribute);
  CHECK(doc.load_string(STR("<node &='1'/>"), parse_minimal).status ==
        status_bad_start_element);
}
  ...
  for (int wnorm = 0; wnorm < 2; ++wnorm)
    for (int eol = 0; eol < 2; ++eol)
      for (int wconv = 0; wconv < 2; ++wconv) {
        ...
        CHECK(doc.load_string(STR("<node id='value"), flags).status ==
              status_bad_attribute);
      }
}
...
TEST(parse_tag_error) {
  xml_document doc;
  CHECK(doc.load_string(STR("<"), parse_minimal).status ==
        status_unrecognized_tag);
  CHECK(doc.load_string(STR("<!"), parse_minimal).status ==
        status_unrecognized_tag);
  CHECK(doc.load_string(STR("<!D"), parse_minimal).status ==
        status_unrecognized_tag);
  CHECK(doc.load_string(STR("<#"), parse_minimal).status ==
        status_unrecognized_tag);
  CHECK(doc.load_string(STR("<node#"), parse_minimal).status ==
  ...
  CHECK(doc.load_string(STR("</node "), parse_minimal).status ==
        status_end_element_mismatch);
  CHECK(doc.load_string(STR("<node></ node>"), parse_minimal).status ==
        status_end_element_mismatch);
  CHECK(doc.load_string(STR("<node></node"), parse_minimal).status ==
        status_bad_end_element);
  CHECK(doc.load_string(STR("<node></node "), parse_minimal).status ==
        status_bad_end_element);
  CHECK(doc.load_string(STR("<node></nodes>"), parse_minimal).status ==
        status_end_element_mismatch);
  CHECK(doc.load_string(STR("<node>"), parse_minimal).status ==
        status_end_element_mismatch);
  CHECK(doc.load_string(STR("<node/><"), parse_minimal).status ==
        status_unrecognized_tag);
  CHECK(doc.load_string(STR("<node attr='value'>"), parse_minimal).status ==
        status_end_element_mismatch);
  CHECK(doc.load_string(STR("</></node>"), parse_minimal).status ==
        status_end_element_mismatch);
  CHECK(doc.load_string(STR("</node>"), parse_minimal).status ==
  ...
  CHECK(doc.load_string(STR("</>"), parse_minimal).status ==
        status_end_element_mismatch);
  CHECK(doc.load_string(STR("<node></node v>"), parse_minimal).status ==
        status_bad_end_element);
  CHECK(doc.load_string(STR("<node&/>"), parse_minimal).status ==
        status_bad_start_element);
  CHECK(doc.load_string(STR("<node& v='1'/>"), parse_minimal).status ==
        status_bad_start_element);
}
...
TEST(parse_declaration_error) {
  ...
  for (unsigned int i = 0; i < sizeof(flag_sets) / sizeof(flag_sets[0]); ++i) {
    ...
    CHECK(doc.load_string(STR("<?xml?"), flags).status == status_bad_pi);
    CHECK(doc.load_string(STR("<?xml>"), flags).status == status_bad_pi);
    CHECK(doc.load_string(STR("<?xml version='1>"), flags).status ==
          status_bad_pi);
  }

  CHECK(doc.load_string(STR("<?xml version='1?>"),
                        parse_fragment | parse_declaration)
            .status == status_bad_attribute);
  CHECK(doc.load_string(STR("<foo><?xml version='1'?></foo>"),
                        parse_fragment | parse_declaration)
            .status == status_bad_pi);
}
...
#define CHECK_OFFSET(contents, options, status, offset) \
  CHECK(test_offset(STR(contents), options, status, offset))
...
TEST(parse_error_offset) {
  ...
  CHECK_OFFSET(
      ...
      "\"orde",
      parse_default, status_bad_doctype, 76);
...
TEST(parse_embed_pcdata) {
  ...
  for (int i = 0; i < 2; ++i) {
    unsigned int flags =
        (i == 0) ? parse_default : parse_default | parse_embed_pcdata;

    xml_document doc;
    xml_parse_result res =
        doc.load_string(STR("<node><key>value</key><child><inner1>value1</"
    ...
    if (flags & parse_embed_pcdata) {
      CHECK_STRING(doc.child(STR("node")).child(STR("key")).value(),
                   STR("value"));
      ...
      CHECK(doc.child(STR("node")).child(STR("key")).first_child());
      CHECK_STRING(
          doc.child(STR("node")).child(STR("key")).first_child().value(),
          STR("value"));
    }
...
TEST_XML_FLAGS(parse_embed_pcdata_fragment,
               "text",
...
TEST_XML_FLAGS(parse_embed_pcdata_child,
               "<n><child/>text</n>",
               parse_embed_pcdata) {
...
TEST_XML_FLAGS(parse_embed_pcdata_comment,
               "<n>text1<!---->text2</n>",
               parse_embed_pcdata) {
  xml_node n = doc.child(STR("n"));
...
TEST(parse_merge_pcdata) {
  unsigned int flag_sets[] = {parse_cdata, parse_pi, parse_comments,
                              parse_declaration};
  ...
  for (unsigned int i = 0; i < sizeof(flag_sets) / sizeof(flag_sets[0]); ++i) {
    ...
    xml_document doc;
    xml_parse_result res =
        doc.load_string(STR("<node>First text<!-- here is a mesh node "
    ...
    if (flags & parse_comments) {
      ...
      CHECK_NODE(doc, STR("<node>First text<!-- here is a mesh node -->Second "
                          "textsome more textLast text</node>"));
      ...
      CHECK_NODE(doc,
                 STR("<node>First textSecond text<![CDATA[someothertext]]>some "
                     "more textLast text</node>"));
    } else if (flags & parse_pi) {
      CHECK_STRING(child.first_child().value(),
                   STR("First textSecond textsome more text"));
      CHECK(child.first_child().next_sibling().type() == node_pi);
      CHECK_NODE(doc, STR("<node>First textSecond textsome more text<?include "
                          "somedata?>Last text</node>"));
    } else {
      CHECK(child.first_child() == child.last_child());
      CHECK(child.first_child().type() == node_pcdata);
      CHECK_NODE(
          doc,
          STR("<node>First textSecond textsome more textLast text</node>"));
    }
...
TEST(parse_merge_pcdata_escape) {
  xml_document doc;
  xml_parse_result res = doc.load_string(
...
TEST(parse_merge_pcdata_whitespace) {
  ...
  for (unsigned int i = 0; i < sizeof(flag_sets) / sizeof(flag_sets[0]); ++i) {
    ...
    xml_document doc;
    xml_parse_result res =
        doc.load_string(STR("<node><child1>  <!-- comment 1 -->\t<!-- comment "
    ...
    if (flags & parse_ws_pcdata) {
      CHECK_STRING(doc.child(STR("node")).child(STR("child1")).child_value(),
                   STR("  \t\n"));
      CHECK_STRING(doc.child(STR("node")).child(STR("child2")).child_value(),
                   STR("text\t end"));
