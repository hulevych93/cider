...
static void overflow_hash_table(xml_document& doc) {
  ...
  // compact encoding assumes next_sibling is a forward-only pointer so we can
  // allocate hash entries by reordering nodes we allocate enough hash entries
  // to be exactly on the edge of rehash threshold
  for (int i = 0; i < 8; ++i)
