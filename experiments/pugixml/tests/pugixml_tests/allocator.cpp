...
void* allocate_page_aligned(size_t size) {
  // We can't use VirtualAlloc because it has 64Kb granularity so we run out of
  // address space quickly We can't use malloc because of occasional problems
  // with CW on CRT termination
  static HANDLE heap = HeapCreate(0, 0, 0);

  void* result = HeapAlloc(heap, 0, size + page_size);

  return reinterpret_cast<void*>(
      align_to_page(reinterpret_cast<size_t>(result)));
}
...
void deallocate(void* ptr, size_t size) {
  ...
  VirtualProtect(rptr, aligned_size + page_size, PAGE_NOACCESS, &old_flags);
    ...
    (defined(__i386) || defined(__x86_64)) && !ADDRESS_SANITIZER
#include <sys/mman.h>
...
const size_t memory_alignment = sizeof(double) > sizeof(void*) ? sizeof(double)
                                                               : sizeof(void*);
...
void memory_deallocate(void* ptr) {
  ...
  deallocate(static_cast<char*>(ptr) - memory_alignment,
