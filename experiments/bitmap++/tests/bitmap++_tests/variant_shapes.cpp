...
struct Circle {
  ...
  void draw(Bitmap& image) const { image.fill_circle(x, y, radius, color); }
};

struct ShapeDrawer {
  Bitmap& bitmap;

  explicit ShapeDrawer(Bitmap& bitmap) : bitmap(bitmap) {}
...
int test_variant_shapes() {
  try {
    ...
    // for more details, watch this great talk by Klaus Iglberger:
    // https://www.youtube.com/watch?v=m3UmABVf55g
    using Shape = std::variant<Rectangle, Triangle, Circle>;
    Shape shapes[] = {Rectangle(20, 20, 180, 180, Gold),
                      Triangle(310, 20, 230, 200, 400, 200, Violet),
                      Circle(500, 110, 90, Coral)};

    for (Shape& shape : shapes) {
      std::visit(drawer, shape);  // visit will call the appropriate
                                  // ShapeDrawer::operator() for each shape
    }
