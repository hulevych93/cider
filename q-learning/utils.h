...
public:
    AdvancedAdaptiveLearningRate(float initial_lr = 0.1,
                                 float min_lr = 0.05,
                                 float max_lr = 0.3,
                                 float increase_factor = 1.05,
                                 float decrease_factor = 0.95,
                                 int patience = 50,
    ...
    void adapt(float current_loss, float current_reward) {
        ...
        if (std::fabs(average_reward - current_reward) < reward_stability_threshold) {
            learning_rate = std::min(max_lr, learning_rate * 1.1f);
            std::cout << "Stable reward, incresing Learning Rate: " << learning_rate << std::endl;
        }
