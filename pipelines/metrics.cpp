...
Metrics computeMetrics(const std::vector<Result>& results) {
    ...
    std::vector<double> oldCovs, newCovs, oldCfgs, newCfgs, oldLens, newLens, times;
    ...
    if (n > 0) {
        ...
        m.avgOldCov = mean(oldCovs);  m.stdOldCov = computeStdDev(oldCovs, m.avgOldCov);
        m.avgNewCov = mean(newCovs);  m.stdNewCov = computeStdDev(newCovs, m.avgNewCov);
        m.avgOldCfg = mean(oldCfgs);  m.stdOldCfg = computeStdDev(oldCfgs, m.avgOldCfg);
        m.avgNewCfg = mean(newCfgs);  m.stdNewCfg = computeStdDev(newCfgs, m.avgNewCfg);
        m.avgOldLen = mean(oldLens);  m.stdOldLen = computeStdDev(oldLens, m.avgOldLen);
        m.avgNewLen = mean(newLens);  m.stdNewLen = computeStdDev(newLens, m.avgNewLen);
        m.avgTime   = mean(times);    m.stdTime   = computeStdDev(times, m.avgTime);

        m.compression = (m.avgOldLen - m.avgNewLen) / m.avgOldLen;
        m.covDelta = m.avgNewCov - m.avgOldCov;
        m.cfgDelta = m.avgNewCfg - m.avgOldCfg;

        m.effScore = m.covDelta + m.compression
                     - 0.1 * (m.avgTime / 1000.0)
