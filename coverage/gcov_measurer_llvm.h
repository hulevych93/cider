...
#include "coverage/gcov_coverage_llvm.h"
...
using ReportOpt = std::optional<gcov_coverage::RootReport>;

struct CoverageMeasurment {
  CoverageMeasurment(const Cmd& cmd, const char* module);

  void setLogger(gcov_coverage::ICoverageLogger* logger) { m_logger = logger; }
 ...
 protected:
  ...
  const char* _module;
  gcov_coverage::ICoverageLogger* m_logger = nullptr;
};
