  ...
  try {
    if (argc != 2) {
      ...
      cppast::libclang_compilation_database database(
          argv[1]);  // the compilation database

      // simple_file_parser allows parsing multiple files and stores the results
      // for us
      cppast::simple_file_parser<cppast::libclang_parser> parser(
          type_safe::ref(index));
      try {
        cppast::parse_database(parser,
                               database);  // parse all files in the database
...
void print_ast(const cppast::cpp_file& file) {
  ...
  cppast::visit(
      file, [&](const cppast::cpp_entity& e, cppast::visitor_info info) {
        if (info.event ==
            cppast::visitor_info::container_entity_exit)  // exiting an old
                                                          // container
          prefix.pop_back();
        ...
        // entering a new container
        {
          ...
          prefix += "\t";
        } else  // if (info.event == cppast::visitor_info::leaf_entity) // a
                // non-container entity
          std::cout << prefix << "'" << e.name() << "' - "
                    << cppast::to_string(e.kind()) << '\n';
      });
}
