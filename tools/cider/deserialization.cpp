...
void printAggregateSerializeField(std::ostream& os, const std::string& name) {
  os << "deserializer >> aggregate." << name << ";\n";
}
...
void printAggregateSerializer(std::ostream& os,
  ...
  if (enter) {
    os << "template <>\n";
    os << "bool deserializeAggregate(" << scope + "::" + e.name()
       << "& aggregate, const serialization::Deserializer& deserializer) {\n";
...
void printEnumSerializer(std::ostream& os,
  ...
  if (enter) {
    os << "template <>\n";
    os << "bool deserializeAggregate(" << scope + "::" + name
       << "& aggregate, const serialization::Deserializer& deserializer) {\n";
    os << "deserializer >> aggregate;\n";
  } else {
    os << "return true;\n";
    os << "}\n\n";
  }
...
deserialization_generator::deserialization_generator(
    std::ostream& out,
...
void deserialization_generator::handleClass(
...
void deserialization_generator::handleMemberVariable(
...
void deserialization_generator::handleEnum(const cppast::cpp_enum& e,
                                           const bool enter) {
...
void deserialization_generator::handleNamespace(const cpp_entity& e,
                                                const bool enter) {
  if (enter) {
    m_namespaces.push(null_stream, e.name());
  } else {
